...
#: Restored from the Baseline commit per CLAUDE.md, with three surgical changes and no
#: others: the backend list is now a parameter (it was hardcoded to the two classical
#: engines, which predate the neural ones), the handheld photo is included (it is the
#: only genuine camera capture in the fixture set and the degraded case that matters
#: most), and JPEG is a known media type so that photo can be read at all.
DEFAULT_BACKENDS = ("textlayer", "tesseract", "got-ocr2")

#: `{base}{suffix}` for every fixture family. `_photo_handheld` is the real photograph.
VARIANT_SUFFIXES = (".pdf", "_scan.png", "_degraded.png", "_photo_handheld.jpg")
...
def score_one(backend_name: str, path: Path, truth: dict) -> Score:
    variant = (
        "handheld"
        if "_photo_handheld" in path.stem
        else "degraded"
...
def run(backends: tuple[str, ...] = DEFAULT_BACKENDS) -> list[Score]:
...
def main() -> int:
    chosen = tuple(sys.argv[1].split(",")) if len(sys.argv) > 1 else DEFAULT_BACKENDS
    scores = run(chosen)
    print(render(scores))
